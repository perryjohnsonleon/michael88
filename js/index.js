	...
	const MAIN = { sym: '大盤指數', id: '2353', price: 0, open:0, high: 0, low: 0, change: 0, amt:0};
	const MARKETS = [list1,list2,list3,list4,list5,list6,list7,list8,list9,list10,list11];
	const state = {
	  main: { ...MAIN, open: MAIN.price, high: MAIN.price, low: MAIN.price, change: 0 , flat:0 , amt:0},
  ...
  async function getPost(stockId) {
	  try {
		let fetchUrl_str="" ;
		let fetchUrl_str1="http://kgieworld.moneydj.com/z/bcd/GetStkRTDataJSON.djjson?B=" , fetchUrl_str2="&xyz=86400"   ;
		if (stockId == 9999 ) 
			fetchUrl_str="https://www.kgi.com.tw/api/kgis/SysJustData/GetStockQuote?sc_lang=zh-TW&amp;sc_site=kgis-zh-tw"
		else
	  ...
	  if (post) {
			...
			ITEM2.forEach((quote_obj,idx) => {
				for ( var n in quote_obj) {
				  ...
				  row.appendChild(gainCell);
				  [MAIN.open,MAIN.high,MAIN.low,MAIN.amt].forEach(value => {
					const cell = document.createElement('div');
					cell.className = 'item3w';
					cell.textContent = value;
					row.appendChild(cell);
 ...
 async function displayPost(stockId,itemId) {
	  ...
	  if (stockId == 9999) {
			if (post) {
				const quote_obj = post.Result.ResultSet.Result ;	
				for ( var n in quote_obj) {
					if ( n == "V4" ) {
						document.getElementById("wi-c").innerHTML = quote_obj[n]  ;		
					}
					if ( n == "V5" ) {
						if (quote_obj[n]> 0) 
                            {
  ...
  async function displayPostChart() {
   ...
   function timestampToTime(timestamp) {
	...
	async function alarmPrice(stockNo) {
		window.location.href = 'http://127.0.0.1:8000/winder/pricealarm.htm?stockid=' + stockNo ;
    }	
